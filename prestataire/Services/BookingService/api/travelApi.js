...
const updateTravel = async (id) => {
  axios
    .post(`${process.env.TRAVEL_ADDR}/deleteTravel`, { idTravel: id })
    .catch((error) => console.log("Error on updating travel data", error));
};

const getPrice = async (body) => {
  return await axios
    .post(`${process.env.PRICE_ADDR}/price`, body)
...
const getLinkPayement = async (body) => {
  return await axios
    .post(`${process.env.PAYMENT_ADDR}/payment`, body)
    .then((response) => {
      return response.data;
    })
    .catch((error) => console.log("Error to get the payment link", error));
};
